<%@ Page Title="โปรโมชั่น โรงพยาบาลกรุงเทพจันทบุรี" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Promotion.aspx.cs" Inherits="Promotion" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBody" Runat="Server">
    ...
    <div id="dvHeader" runat="server">
        <div style="float:left;margin-right:10px;">
            <asp:Label ID="lblIcon" runat="server" Text="<img src='/Images/icArticle.png'/>"/>
        </div>
        <div style="float:left;width:800px;">
            <div style="text-align:left;">
                <div>
                    <asp:Label ID="lblName" runat="server" Text="<h1>โปรโมชั่น</h1>"/>
                </div>
                <div>
                    <asp:Label ID="lblDetail" runat="server" Text="ข้อมูลโปรโมชั่นของโรงพยาบาลกรุงเทพจันทบุรี"/>
                </div>
        ...
        <div style="float:right;padding:5px;">
            <asp:Panel ID="pnManage" runat="server" Visible="false">
                <a href="/PromotionManage.aspx" class="cbIFrame"><span class="Button AddTH"></span></a>
            </asp:Panel>
        </div>
                        ...
                        <ItemTemplate>
                            <div class="gvRow" style='background-color:<%#(AnonymousViewChecker(DataBinder.Eval(Container.DataItem,"ActiveDateFrom").ToString(),DataBinder.Eval(Container.DataItem,"ActiveDateTo").ToString(),DataBinder.Eval(Container.DataItem,"StatusFlag").ToString())?"#FFF":"#DDD;") %>'>
                                ...
                                <div class="gvCell" style="width:100%;padding-left:10px;">
                                    <h3>
                                        <a href='/Promotion/<%#DataBinder.Eval(Container.DataItem,"UID") %>/<%#clsDefault.URLRoutingFilter(DataBinder.Eval(Container.DataItem,"Subject")) %>/'><%#DataBinder.Eval(Container.DataItem, "Subject")%></a>
                                        <span style='display:<%#(clsSecurity.LoginChecker("admin")?"inline":"none") %>'>
                                            <a href="/PromotionManage.aspx?id=<%#DataBinder.Eval(Container.DataItem,"UID") %>&command=edit" class="cbIFrame"><span class="Icon16 Edit"></span></a>
                                            <a href="/PromotionManage.aspx?id=<%#DataBinder.Eval(Container.DataItem,"UID") %>&command=delete" onclick="return confirm('กดปุ่ม OK เพื่อยืนยันการลบข้อมูล')"><span class="Icon16 Delete"></span></a>
                                        </span>
                                    </h3>
                                    <div>
