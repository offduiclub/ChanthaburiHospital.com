...
public class clsLanguage
{
    ...
    private int _languageUIDCurrent;
    public int LanguageUIDCurrent
    {
        get 
        {
            try
            {
                ...
                _languageUIDCurrent = int.Parse(clsSQL.Return("SELECT UID FROM Language WHERE Name='" + LanguageCurrent + "'", clsSQL.DBType.SQLServer, "cs"));
            }
            catch (Exception ex)
            {
                _languageUIDCurrent = 0;
            }
            return _languageUIDCurrent; 
        }
    }
