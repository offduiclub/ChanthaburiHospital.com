...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBodyHead" Runat="Server">
    ...
    <div style="text-align:left;float:left;">
        ...
        ระบบจัดการข้อมูลศูนย์สุขภาพ | <a href="<%=clsDefault.QueryStringChecker("globalpage")+clsDefault.QueryStringRemover(new string[]{"globalid","globalname","globalpage"}) %>">กลับสู่หน้าหลัก</a>
    </div>
    <div style="float:right;margin-top:17px;margin-right:9px;">
        <a href="/Management/<%=webManage %>?globalid=<%=clsDefault.QueryStringChecker("globalid") %>&globalname=<%=clsDefault.QueryStringChecker("globalname") %>" class="cbIFrame">
                        ...
                        <ItemTemplate>
                            <div class="dvPhotoFrame">
                                <div class="dvPhotoFrameControl">
				                    ...
				                    <a onclick="return confirm('กดปุ่ม OK เพื่อยืนยันการลบข้อมูล')" href="/Management/<%=webManage %><%#clsDefault.QueryStringMerge(new string[,]{{"id",DataBinder.Eval(Container.DataItem,"UID").ToString()},{"command","delete"}}) %>" class="Icon16 Delete" title="ลบข้อมูล"></a>
                                </div>
                                <div class="dvPhotoFramePhoto">
                                    <a href='<%#DataBinder.Eval(Container.DataItem,"Photo") %>' class="cbPhoto">
                                        <img src="<%#DataBinder.Eval(Container.DataItem, "PhotoPreview")%>" alt="<%#DataBinder.Eval(Container.DataItem,"Name") %>" title="<%#DataBinder.Eval(Container.DataItem,"Name") %>"/>
                                    </a>
                                </div>
